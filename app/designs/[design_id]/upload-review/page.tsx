...
export default function DesignUploadReviewPage() {
  const router       = useRouter();
  const { design_id } = useParams<{ design_id: string }>();

  const [data, setData]   = useState<DesignUploadData | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (res.ok && active) setData(await res.json() as DesignUploadData);
  ...
  if (!ready) {
    return (
      <BriefLayout currentStep={3} title="Review Your File">
  ...
  return (
    <BriefLayout
      currentStep={3}
      title="Review Your File"
      subtitle="Confirm your uploaded design file before activating your project."
    >
      <div className="space-y-6">
        ...
        <div className="bg-brand-surface rounded-xl border border-brand-border p-5">
          ...
          {data?.teamName && <InfoRow label="Team"  value={data.teamName} />}
          {data?.sport    && <InfoRow label="Sport" value={data.sport} />}
        </div>

        {/* Notes */}
        {data?.clientConceptNotes && (
          <div className="bg-brand-surface rounded-xl border border-brand-border p-5">
        ...
        <div className="flex gap-3 pt-2">
          ...
          {conceptUrl && (
            <a
              href={`/designs/${design_id}/checkout`}
