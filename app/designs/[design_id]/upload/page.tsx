...
export default function DesignUploadPage() {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const res  = await fetch(`/api/designs/${design_id}/upload-concept`, { method: "POST", body: formData });
      const data = await res.json() as { url?: string; designId?: string; error?: string };

      if (!res.ok || !data.url) {
        throw new Error(data.error ?? "Upload failed. Please try again.");
      }

      router.push(`/designs/${design_id}/upload-review`);
    } catch (err) {
        ...
        <div className="w-full max-w-lg space-y-7">
          ...
          <div>
            ...
            <div
              ...
              `}
            >
              <input ref={inputRef} type="file" accept={DESIGN_ACCEPTED} onChange={handleInputChange} className="hidden" />
          ...
          <div>
            ...
            <div
              ...
              `}
            >
              <input ref={photoInputRef} type="file" accept={PHOTO_ACCEPTED} onChange={handlePhotoInputChange} className="hidden" />
          ...
          <div className="rounded-2xl border border-brand-border bg-brand-surface px-5 py-4">
            <p className="text-[9px] font-display font-bold uppercase tracking-[0.28em] text-brand-muted mb-3">What happens next</p>
            <ol className="space-y-3">
              {[
                "Upload your production file (you're here)",
                "Creative Activation — $149 applied to your final order total",
                ...
                <li key={i} className="flex items-start gap-3">
                  <span className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-[9px] font-display font-bold ${i === 0 ? "bg-brand-primary text-white" : "bg-brand-border text-brand-muted"}`}>
                    {i + 1}
                  </span>
          ...
          >
            {uploading
              ? "Uploading…"
              : file && photo
                ? "Upload → Review Before Activation"
                : !file
